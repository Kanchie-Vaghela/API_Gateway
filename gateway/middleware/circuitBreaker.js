...
class CircuitBreaker {
  ...
  recordSuccess() {
    ...
    this.state = "CLOSED";
    this.openedAt = null;
    circuitBreakerState.set({ service: this.name }, stateToNum["CLOSED"]);
  }

  recordFailure() {
    this.failureCount++;
    console.log(
      `[circuit-breaker] ${this.name}: failure ${this.failureCount}/${this.threshold}`,
    );
    ...
    if (this.state === "HALF_OPEN") {
      ...
      circuitBreakerState.set({ service: this.name }, stateToNum["OPEN"]);
      return;
    }
